...
// by thearst3rd and CaptainRektbeard
...
update
{
	...
	if (vars.gameStateOffset == null)
	{
		...
		if (vars.hasEnteredLevel)
		{
			...
			{
				IntPtr gameStatePtr;
				new DeepPointer(game.MainModule.BaseAddress + 0x294A84, offset * 4, 0x0c, 0x00).DerefOffsets(game,
						out gameStatePtr);
	...
	else // Otherwise just use the offset to read the string
	{
		IntPtr gameStatePtr;
		new DeepPointer(game.MainModule.BaseAddress+0x294A84, (int)vars.gameStateOffset * 4, 0x0c, 0x00)
				.DerefOffsets(game, out gameStatePtr);
	...
	if (current.levelFilename.StartsWith("/data/missions/") && (current.levelFilename != old.levelFilename
			|| vars.firstUpdate))
	{
		vars.Log("Entering level: " + vars.ExtractLevelType(current.levelFilename) + " - " +
				vars.ExtractLevelName(current.levelFilename));
...
split
{
	...
	{
		string settingName = vars.ExtractLevelType(current.levelFilename) + "_" +
				vars.ExtractLevelName(current.levelFilename);
		vars.Log("Finished level: " + settingName);
